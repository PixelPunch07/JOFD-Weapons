...
SWEP.VElements = {
	["cube+"] = { type = "Model", model = "models/props_questionableethics/qe_primarylaser.mdl", bone = "Base", rel = "", pos = Vector(0, 18.181, 2.596), angle = Angle(-90, 90, 0), size = Vector(0.2, 0.2, 0.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "freak_fortress_2/captainnrb/kahlmstein_body1", skin = 0, bodygroup = {} },
	["fists"] = { type = "Model", model = "models/weapons/c_models/c_fists_of_steel/c_fists_of_steel.mdl", bone = "Base", rel = "", pos = Vector(8.831, -0.519, 4.675), angle = Angle(180, 22.208, 87.662), size = Vector(0.5, 0.5, 0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} },
	["gaster++"] = { type = "Model", model = "models/evangelos/undertale/gasterblaster.mdl", bone = "Base", rel = "", pos = Vector(-1.558, -0.519, 30.649), angle = Angle(108.7, -90, 0), size = Vector(0.958, 0.958, 0.958), color = Color(255, 255, 255, 255), surpresslightning = false, material = "freak_fortress_2/captainnrb/kahlmstein_body1", skin = 9, bodygroup = {} },
	["glasses"] = { type = "Model", model = "models/player/items/all_class/dex_glasses_heavy.mdl", bone = "Base", rel = "", pos = Vector(0, 0.518, -8.832), angle = Angle(-43.248, 5.843, -87.663), size = Vector(1.598, 1.598, 1.598), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} },
	["brass"] = { type = "Model", model = "models/weapons/c_models/c_gatling_gun/c_gatling_gun.mdl", bone = "Base", rel = "", pos = Vector(-9.87, -5.715, 6.752), angle = Angle(-80.65, 24.545, -134.417), size = Vector(0.301, 0.301, 0.301), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["brass"] = { type = "Model", model = "models/weapons/c_models/c_gatling_gun/c_gatling_gun.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(7.791, -5.715, -8.832), angle = Angle(1.169, 10.519, 162.468), size = Vector(0.237, 0.237, 0.237), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} },
	["cube"] = { type = "Model", model = "models/props_questionableethics/qe_primarylaser.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(15.817, -0.159, 1.557), angle = Angle(175.675, 10.519, -15.195), size = Vector(0.108, 0.108, 0.108), color = Color(255, 255, 255, 255), surpresslightning = false, material = "freak_fortress_2/captainnrb/kahlmstein_body1", skin = 0, bodygroup = {} },
	["fists"] = { type = "Model", model = "models/weapons/c_models/c_fists_of_steel/c_fists_of_steel.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(14.026, 3.635, -9.87), angle = Angle(26.882, 101.688, 3.506), size = Vector(0.367, 0.367, 0.367), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} },
	["gaster"] = { type = "Model", model = "models/evangelos/undertale/gasterblaster.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(23.377, -4.676, -9.87), angle = Angle(-26.883, 17.531, -162.469), size = Vector(0.497, 0.497, 0.497), color = Color(255, 255, 255, 255), surpresslightning = false, material = "freak_fortress_2/captainnrb/kahlmstein_body1", skin = 0, bodygroup = {} },
	["glasses"] = { type = "Model", model = "models/player/items/all_class/dex_glasses_heavy.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(27.531, -2.597, -1.558), angle = Angle(-19.871, 17.531, -162.469), size = Vector(1.728, 1.728, 1.728), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
...
function SWEP:Deploy()
	...
	self.Owner:EmitSound(Sound("weapons/gblaster/new_intro.mp3"))
	self:SetIronsights(false, self.Owner)  
	self.Owner:SetFOV( 0, 0.5 )
	
	self.BeatSound = CreateSound( self.Owner, Sound( "weapons/gblaster/kahml_holding.mp3" ) )
	self.BeatSound = CreateSound( self.Owner, Sound( "weapons/gblaster/kahml_holding.mp3" ) )
...
local Mat = Material( "z_easyskins/camo/i_need_to_stop_making_animated_textures" )
function SWEP:PrintWeaponInfo( x, y, alpha )
	if self.InfoMarkup == nil then
		local str, title_color, text_color = "", "<color=255,255,255,255>", "<color=200,200,200,255>"
		str = "<font=TargetID>"
		if ( self.Author != "" ) then str = str..title_color..language.GetPhrase( "Kahmlstein" )..": </color>"..text_color..self.Author.."</color>\n" end
		if ( self.Purpose != "" ) then str = str..title_color..language.GetPhrase( "EVERYONE! As promised, I'm BACK! Did you miss me? Yes you DID! Are you ready to die? Ya? Good." ) end
		if ( self.Instructions != "" ) then str = str..title_color.."			- Kahmlstein" end
...
function SWEP:PrimaryAttack()
	...
	if self.Owner:KeyDown(IN_ATTACK) and self.CandoSpecial == true then
		...
		if self.Owner:IsPlayer() then
			if ( self.LoopSound ) then
				...
				self.LoopSound = CreateSound( self.Owner, Sound( "weapons/gblaster/heavy_terror_rage.mp3" ) )
	...
	if self.Owner:IsNPC() then
		...
		if ( self.LoopSound ) then
			...
			self.LoopSound = CreateSound( self.Owner, Sound( "weapons/gblaster/heavy_terror_rage.mp3" ) )
			self.LoopSound = CreateSound( self.Owner, Sound( "weapons/gblaster/kahml_firing.mp3" ) )
			if ( self.LoopSound ) then self.LoopSound:Play() end
		end
		if ( self.BeatSound ) then self.BeatSound:ChangeVolume( 0, 0.1 ) end
	end
	...
	bullet.Damage = self.Primary.Damage
	bullet.AmmoType = self.Primary.Ammo
	bullet.TracerName = "gravrifle_preexplode"
	bullet.Callback = function(attacker, trace, dmginfo)
					local g = math.random( 1, 3 )
					if ( g == 1 ) then
					dmginfo:SetDamageType(DMG_AIRBOAT)
...
function SWEP:Holster()
	...
	if CLIENT and IsValid(self.Owner) then
		...
		self.Owner:EmitSound(Sound("weapons/gblaster/gardened2.mp3"))
	end
	return true
end
