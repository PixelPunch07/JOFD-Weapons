...
SWEP.Purpose			= "Little bit of coffee to wake you up."
...
function SWEP:Deploy()
	...
	self.BeatSound = CreateSound( self.Owner, Sound( "weapons/gblaster/bullethell_hold.mp3" ) )
...
function SWEP:PrimaryAttack()
	...
	if self.Owner:KeyDown(IN_RELOAD) then
	     if self.Owner:GetNWInt("coffee") >= 30000 then
			self.CandoSpecial = true
		end
	end
	
	if self.Owner:KeyDown(IN_ATTACK) and self.CandoSpecial == true and self.Owner:GetNWInt("coffee") >= 30000 then
		if SERVER then
			local trace = {}
			...
			if timer.TimeLeft( "qe_gasterblaster2"..self:EntIndex() ) == nil then
				...
				local ent = ents.Create( "ent_suddenblaster" )
			...
			timer.Create( "qe_gasterblaster2"..self:EntIndex(), 0.6, 1, function() end )	
			end
		end
		if self.Owner:IsPlayer() then
			if ( self.LoopSound2 ) then
				...
				self.LoopSound2 = CreateSound( self.Owner, Sound( "weapons/gblaster/bullethell_fire.mp3" ) )
	...
	if ( self.Owner:IsNPC() ) then
		...
		if ( self.LoopSound ) then
			...
			self.LoopSound = CreateSound( self.Owner, Sound( "weapons/gblaster/bullethell_fire.mp3" ) )
	...
	self.Owner:EmitSound(Sound("weapons/gblaster/minigun_firing.mp3"))
...
function SWEP:SecondaryAttack()
 local currentcoffee = self.Owner:GetNWBool("coffee")
 if currentcoffee >= 30000 then
 self.Owner:SetHealth(self.Owner:Health() + 20000)
 self.Owner:SetNWInt("coffee", 0) 
 self.Owner:EmitSound(Sound("weapons/gblaster/healing.mp3"))
 timer.Create( "HRRRNG", 0.5, 1, function() self.Owner:SetRunSpeed(self.Owner:GetRunSpeed() +500) end )
 timer.Create( "slowagain", 5, 1, function() self.Owner:SetRunSpeed(self.Owner:GetRunSpeed() -500) end )
...
function SWEP:DrawHUD()
	...
	local suddentag = "weapon_bullethell"
			
function Coffeemoment(ply, dmginfo)
    local attacker = dmginfo:GetAttacker()
    if attacker:IsPlayer() and ply:IsPlayer() and attacker != ply and attacker:GetActiveWeapon():GetClass() == suddentag and attacker:GetNWInt("coffee") <= 30000 then
        local damage = dmginfo:GetDamage()
       attacker:SetNWInt("coffee", attacker:GetNWInt("coffee") + 52 )
	           local chance = math.random(1, 60)
			   
	             if chance == 40 and attacker:GetNWInt("coffee") >= 15000 then
	   			local ent = ents.Create( "ent_suddenblaster" )
				local rand = math.Rand( -math.pi, math.pi ) * 2
				...
				ent:SetAngles( ( ply:GetPos() - pos ):Angle() + Angle(-8,0,0) )
