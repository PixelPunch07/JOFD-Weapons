...
SWEP.Purpose			= "How do you expect to kill death with death?"
...
hook.Add("PlayerSpawn", "GOD_Loading", function(ply)-- tha valueess
    if IsValid(ply) then

	ply:SetNWInt("DECAY", 0) 
	ply:SetNWInt("VOID", 0) 
	ply:SetNWInt("MaxDecay", 50000) 
	ply:SetNWInt("MaxVoid", 10000) 
...
local godtag = "weapon_god"
			
function PlayerLifesteal(ply, dmginfo)
    local attacker = dmginfo:GetAttacker()
    if attacker:IsPlayer() and ply:IsPlayer() and attacker != ply and attacker:GetActiveWeapon():GetClass() == godtag then
        local damage = dmginfo:GetDamage()
		if attacker:GetNWInt("DECAY") < ply:GetNWInt("MaxDecay") then
          attacker:SetNWInt("DECAY", attacker:GetNWInt("DECAY") + 15)  
		  end
		  if attacker:GetNWInt("VOID") < ply:GetNWInt("MaxVoid") then
          attacker:SetNWInt("VOID", attacker:GetNWInt("VOID") + 2)  
		  end
		    end
        end
...
function SWEP:DrawHUD()
    ...
    local ce = ply:GetNWInt("DECAY")
    local maxCE = ply:GetNWInt("MaxDecay")
    local void = ply:GetNWInt("VOID")
    local maxvoid = ply:GetNWInt("MaxVoid")

local Texture1 = Material("blasters/hourglass") 
surface.SetMaterial(Texture1)
surface.SetDrawColor(Color(255, 255, 255, 255))
surface.DrawTexturedRectRotated( ScrW()-1846, ScrH()-575, 455, 715, 0 )
	...
	local voidPercent = void / maxvoid 
	


    local text = "DECAY: " .. ce .. " / " .. maxCE
    local text2 = "VOID: " .. void .. " / " .. maxvoid
    surface.SetFont("8bitoperator")
    local textWidth, textHeight = surface.GetTextSize(text)
    surface.SetDrawColor(0, 0, 0, 60)
    surface.SetDrawColor(0, 58, 255, 200)
    surface.DrawRect(barX, barY, barWidth, barHeight * cePercent)
	

    surface.SetTextColor(0, 58, 255)
    surface.SetTextPos(barX + -28 / 2, barY - textHeight + 425)
    surface.DrawText(text)
	
	surface.SetFont("8bitoperator")
    local textWidth, textHeight = surface.GetTextSize(text)
    surface.SetDrawColor(0, 0, 0, 60)
    surface.SetDrawColor(0, 58, 55, 200)
    surface.DrawRect(barX + 65, barY, barWidth, barHeight * voidPercent)
	

    surface.SetTextColor(5, 5, 5)
    surface.SetTextPos(barX + -28 / 2, barY - textHeight + 453)
	...
	draw.SimpleText("Auto Slash: RMB", "8bitoperator", ScrW() * 0.005, ScrH() * 0.78, Color( 255, 65, 65, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	draw.SimpleText("Blaster/Bone Barrage: R + LMB", "8bitoperator", ScrW() * 0.005, ScrH() * 0.824, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	draw.SimpleText("Mode Switch: R + RMB", "8bitoperator", ScrW() * 0.005, ScrH() * 0.803, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	...
	draw.SimpleText("Passive: You have the ability to switch modes (as said)", "8bitoperator", ScrW() * 0.005, ScrH() * 0.87, Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	draw.SimpleText("Blue mode is defensive, slightly decreasing your walkspeed but giving a blocking mechanic while your slash becomes a sort of line", "8bitoperator", ScrW() * 0.005, ScrH() * 0.89, Color( 0, 255, 255, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
		draw.SimpleText("Red mode is offensive, having higher damage while your slashes become sort of AoEs", "8bitoperator", ScrW() * 0.005, ScrH() * 0.91, Color( 255, 0, 0, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
	end	
