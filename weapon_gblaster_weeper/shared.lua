...
function SWEP:Deploy()
	...
	self.Owner:EmitSound(Sound("weapons/gblaster/weep.wav"))
	self:SetIronsights(false, self.Owner)  
	self.Owner:SetFOV( 0, 0.5 )
	
	self.BeatSound = CreateSound( self.Owner, Sound( "weapons/gblaster/cry.wav" ) )
	...
	if self.Owner:KeyDown(IN_ATTACK) and self.CandoSpecial == true then
		if SERVER then
				...
				if v:IsNPC() or v:IsPlayer() or v.Type == "nextbot" then
					if v:IsValid() and v ~= self and v ~= self.Owner then
						if timer.TimeLeft( "qe_gasterblaster"..self:EntIndex() ) == nil then
							...
							local ent = ents.Create( "ent_qe_cry_blaster" )
						...
						timer.Create( "qe_gasterblaster"..self:EntIndex(), 0.25, 0.5, function() end )	
						end
					end
				end
			end
			...
			if timer.TimeLeft( "qe_gasterblaster2"..self:EntIndex() ) == nil then
				...
				ent:SetPos( self.Owner:GetPos() )
				ent:EmitSound( Sound( "gaster/weeper.mp3" ), 75, 100, 1, CHAN_AUTO )
				ent:SetOwner( self.Owner )
				ent:Spawn()
				ent:SetVar( "position", pos )
...
function SWEP:Holster()
	...
	if CLIENT and IsValid(self.Owner) then
		...
		self.Owner:EmitSound(Sound("weapons/gblaster/woes.wav"))
	end
	return true
end
